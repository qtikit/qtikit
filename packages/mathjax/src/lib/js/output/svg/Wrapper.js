...
var SVGWrapper = (function (_super) {
    ...
    SVGWrapper.prototype.addChildren = function (parent) {
        ...
        for (var i = 0; i < this.childNodes.length; ++i) {
            ...
            var prev = this.childNodes[i - 1];
            child.toSVG(parent);
            if (this.isLineBreakNode(child) && prev && child.element) {
                var top_1 = (prev.bbox.h * prev.bbox.rscale) + 0.18;
                y -= top_1;
                x = 0;
            }
            else {
                if (child.element) {
                    child.place(x + child.bbox.L * child.bbox.rscale, y);
                }
    ...
    SVGWrapper.prototype.childIndexOf = function (child) {
        return this.childNodes.indexOf(child);
    };
    SVGWrapper.prototype.childByIndex = function (index) {
        return this.childNodes[index];
    };
    SVGWrapper.prototype.isLineBreakNode = function (child) {
