import {Document, DOMParser, Element} from 'https://deno.land/x/deno_dom@v0.1.17-alpha/deno-dom-wasm.ts';
...
export default function build(qtiSpecDoc: Document, log?: (message: string) => void): Spec {
...
function getToc(qtiSpecDoc: Document): Toc {
  const typeDefTableLinks = [...qtiSpecDoc.querySelectorAll('#boxIdTable_Type > .tocCaptionLevel1')].map(el => {
...
function getEnum(doc: Document, link: TableLink, errors: Set<string>): Enum | null {
...
function getClass(doc: Document, link: TableLink, errors: Set<string>): Class | null {
  const table = getDefinitionTableFromHash(doc, link.hash, errors);
  if (!table) return null;
  const {name, items} = parseDefinitionTable(table);
  ...
  const characteristics = characteristicsUl ? getFields(doc, characteristicsUl, errors) : {};
...
function getSuperClasses(ul: Element | null): string[] {
  if (!ul) return [];
  return [...ul.querySelectorAll('li')].map(li => li.textContent.split(' ')[0]);
}
...
function* iterFields(doc: Document, ul: Element, errors: Set<string>): Generator<IterFieldsEntry> {
...
function getField(doc: Document, hash: string, errors: Set<string>): Field | null {
...
function getDefinitionElements(doc: Document, hash: string, errors: Set<string>): Element[] {
...
function getDefinitionTableFromHash(doc: Document, hash: string, errors: Set<string>): Element | null {
  const anchor = doc.querySelector(`[name="${hash}"]`);
  if (!anchor) {
    errors.add(hash);
    return null;
...
function getDefinitionTableFromElements(els: Element[]): Element | null {
  return els.reduce<Element | null>((prev, curr) => prev ?? curr.querySelector('table'), null);
}
