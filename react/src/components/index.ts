...
export type InteractionChildElementName =
  typeof interactionChildElementNames[number];
...
export function createInteractionChildComponent(
  ...
  const InteractionChildComponentMap: Record<
    InteractionChildElementName,
    React.FC
  > = {
    prompt: Prompt,
    simpleChoice: SimpleChoice,
  };
  const InteractionChildComponent =
    InteractionChildComponentMap[
      element.nodeName as InteractionChildElementName
    ];

  return InteractionChildComponent
    ? React.createElement(InteractionChildComponent, props, children)
    : null;
}
